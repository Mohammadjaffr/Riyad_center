...
<div class="mb-3">
    ...
    <select name="supplier_id"
        class="summary-input flex-grow-1 w-100 w-md-auto {{ $errors->has('supplier_id') ? 'is-invalid' : '' }}"
...
<div class="mb-3">
    ...
    <input type="date" name="purchase_date"
        class="summary-input flex-grow-1 w-100 w-md-auto {{ $errors->has('purchase_date') ? 'is-invalid' : '' }}"
        style="text-align: right">
    @error('purchase_date')
        <span class="invalid-feedback text-end d-block" role="alert">
            <strong>{{ $message }}</strong>
...
<div class="table-responsive">
    <table class="table table-bordered align-middle text-center custom-invoice-table mb-0 table-striped"
        id="items-table">
        ...
        <tbody>
            <tr class="product-row">
                <td>
                    <select name="variant_id[]" class="summary-input flex-grow-1 w-100 w-md-auto variant-select"
                        style="text-align: right" required>
                        <option disabled selected>اختر متغير</option>
                        @foreach ($products as $product)
                            @foreach ($product->variants as $variant)
                                <option value="{{ $variant->id }}" data-price="{{ $variant->sell_price }}"
                ...
                <td><input type="number" name="quantity[]" class="summary-input flex-grow-1 w-100 w-md-auto quantity"
                        min="1" value="1" required></td>
                <td><input type="number" step="0.01" name="unit_price[]"
                        class="summary-input flex-grow-1 w-100 w-md-auto unit_price" required readonly></td>
                <td><input type="number" step="0.01" name="exchange_rate[]"
                        class="summary-input flex-grow-1 w-100 w-md-auto exchange_rate" value="1"></td>
...
<div class="mb-3">
    ...
    <textarea name="notes"
        class="summary-textarea flex-grow-1 w-100 w-md-auto  {{ $errors->has('notes') ? 'is-invalid' : '' }}"
...
<script>
    ...
    document.addEventListener('input', function(e) {
        if (e.target.matches('.quantity') || e.target.matches('.unit_price') || e.target.matches(
                '.exchange_rate')) {
    ...
    function previewInvoice() {
        document.getElementById('preview_customer_name').textContent = document.querySelector(
            'input[name=customer_name]').value;
        document.getElementById('preview_department').textContent = document.querySelector(
            'select[name=department_id] option:checked').textContent;
        document.getElementById('preview_employee').textContent = document.querySelector(
            'select[name=employee_id] option:checked').textContent;
        document.getElementById('preview_invoice_date').textContent = document.querySelector('input[name=invoice_date]')
            .value;
